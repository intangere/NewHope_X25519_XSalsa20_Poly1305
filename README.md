...
(For the love of everything holy please do not use this in production.)
...
libsodium
```
If you are on a distro with apt-get the following command should be enough:
```
...
Combining this key exchange with more public key algos will offer that much more security because then X amount of algos will have to be broken.

Depending on which key derivation function is used, you could use it as an AES-256+ key. Recommendeded cipher modes would be CTR or GCM.

## Example output
Here is output from a test use of the protocol in example.py
```
...
## Extra
I've added an example implementation that uses your network via Twisted to do the key exchange.
There's a rendezvous server to directly link two clients p2p via UDP hole punching.
So the key exchange takes place with no middle man.
Here's how you can test it (Run each command in a seperate console/terminal window):
...
```
The twisted example is a bare minimum implementation. There is no packet structure and possible errors that will crash twisted as well.
